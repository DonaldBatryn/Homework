...
function balancedParens(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        let obj = str[i];
        let previous = null;
        if (!Object.keys(objs).includes(obj) && !Object.values(objs).includes(obj)) {
            continue;
        } else if (obj in objs) {
            stack.push(obj)
            previous = obj
        // } else if (obj === objs[previous] && stack.length === 0) {
        } else if (Object.values(objs).includes(obj) && stack.length === 0) {
            return false;
        } else if (objs[stack[stack.length - 1]] === obj){
            stack.pop()
        }
    }
