...
def below_average_years
  ...
  years_under_five = Movie.select('yr', 'COUNT(title) AS bad_movies').where('score < ?', 5)
  years_under_five.group('yr').order('yr DESC')
end
...
def uma_movies
  ...
  Actor.joins(:movies).select('movies.id', 'movies.title', 'movies.yr').where('actors.name = ?', 'Uma Thurman').order('yr')
end
